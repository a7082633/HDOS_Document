	...
	{	
		...
		if(m==0x80)//Td1存在  进一步判断ta2
		{
			...
			if((td1&0x10)==0x10)//ta2存在 特定模式
			{
				...
				if((ta2&0x10)!=0x10)//判断b5是否为0，是的话，则使用新的ETU
				{
					if((ta1&0xf0) == 0x10)//判断F是否为1
					{
						switch(ta1&0x0f)
						{
							case 1:ICC_SetCpuETU(ReaderHandle,ICC_Slot_No,0,1);break;
							case 2:ICC_SetCpuETU(ReaderHandle,ICC_Slot_No,0,2);break;
							case 3:ICC_SetCpuETU(ReaderHandle,ICC_Slot_No,0,3);break;
							case 8:ICC_SetCpuETU(ReaderHandle,ICC_Slot_No,0,4);break;
						default:break;
						}
					}
				}
...
long WINAPI ICC_DispSound(long ReaderHandle,unsigned char type,unsigned char nMode) //声音提示信息
{
	if(ReaderHandle<=0)
		return IFD_UnConnected;
